...
\end{verbatimtab}
This phase does the grunt work of the predictive algorithm.  During this phase, pending events that were predicted to happen in the current timestep
are checked to see if they actually occur.  This phase requires additional synchronization overhead for one reason:  collide() reads and writes TWO objects,
which means it could potentially modify objects that occur 
...
\end{verbatimtab}
This algorithm is thread-safe and lock-free provided that the implementations of events.insert and events.find_invalid are thread-safe and lock-free, because predict is a read-only method that 
