...
\subsubsection{find_invalid}

The implementation of find_invalid with our array-based queue is simply a linear search for any events that are marked as invalid in the array.
...
objects from the objects in the current thread.  Although this would seem to break the functionality of find_invalid, this behavior simply serves to reinforce
...
Similar to the implementation of find_invalid, the implementation of find also is implemented based on a linear search.  It finds all valid events
...
This partnership ensures in all the covered cases that the same object data is never used as a concurrent shared resource for both reading and writing.  In addition, the vast majority
of time te
