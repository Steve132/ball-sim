...
PredictiveSimulation::PredictionResult PredictiveSimulation::predict(const Sphere& s1,const Sphere& s2)
{
	PredictiveSimulation::PredictionResult pr;
	pr.collided=false;
	pr.timeoffset=0.0;
	...
	//||s1(t)-s2(t)||==r1+r2
	Vector3d ad=s2.acceleration-s1.acceleration;
	Vector3d vd=s2.velocity-s1.velocity;
	Vector3d sd=s2.position-s1.position;
